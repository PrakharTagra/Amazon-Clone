...
let acc1 = document.querySelector(".accwish");
acc1.addEventListener("mouseover",()=>{
    main.style.opacity="0.4";
})
acc1.addEventListener("mouseout",()=>{
    main.style.opacity="1";
})
...
//   });
 const categorySelect = document.querySelector(".cat");
  ...
  function resizeSelect() {
    ...
    span.style.font = window.getComputedStyle(categorySelect).font;
    span.textContent = categorySelect.options[categorySelect.selectedIndex].text;

    document.body.appendChild(span);
    const width = span.offsetWidth + 40;    
    document.body.removeChild(span);

    categorySelect.style.width = width + "px";
  }

  resizeSelect();
  categorySelect.addEventListener("change", resizeSelect);